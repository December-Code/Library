...
data_xls = pd.read_excel(sys.argv[0], 'records', index_col=None)
...
oldcorr = ['論文名稱(中文)', '論文名稱(外文)', '作者姓名(中文)', '轉檔日期', '類型', '摘要(中文)', '摘要(英文)',
           '中文關鍵詞', '外文關鍵詞', '語文別', '引用網址', '校院名稱', '系所名稱', '學位類別', '指導教授姓名(中文)', ]

newcorr = ['title', 'title:alternative', 'creator', 'date', 'type', 'description:abstract', '',
           'subject1', 'subject2', 'language', 'sys_hyperlink', 'description1', '', 'description2', 'description3']

newcol = ['title', 'title:alternative', 'creator', 'date', 'type', 'description:abstract',
          'subject1', 'subject2', 'language', 'sys_hyperlink', 'description1', 'description2', 'description3']
    ...
    # 不需要合併的 column
        ...
        # 不需要的
            ...
            if nc == '類型':
                ...
                for item in data_xls[nc]:
                    if type(item) is str:
                        item = ILLEGAL_CHARACTERS_RE.sub(
                            r'', item.split('/')[0])
...
df.to_excel(filename, 'Sheet1', encoding='utf-8-sig',
            columns=newcol, index=False)
